    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="bg-white transform -translate-x-1/2 -translate-y-1/2 rounded-2xl p-6 shadow-2xl border border-dourado/20">
              {children}
            </div>
          </motion.div>
        </>
...
const CalendarButton = ({ icon: Icon, label, onClick, className = "" }) => (
  <motion.button
    onClick={onClick}
    className={`flex items-center space-x-3 w-full p-4 rounded-xl border border-dourado/30 hover:bg-bege-claro/50 transition-colors ${className}`}
    whileHover={{ scale: 1.02 }}
    whileTap={{ scale: 0.98 }}
  >
    <Icon className="w-5 h-5" />
    <span className="text-marrom-claro font-sans-modern font-medium">{label}</span>
  </motion.button>
);
...
END:VCALENDAR`;
  ...
  return (
    <div className="relative">
      <motion.div
        className="bg-white rounded-2xl p-6 shadow-sm border border-dourado/20 space-y-4"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
      >
        <div className="flex justify-between items-center">
          <h3 className="text-xl font-serif-elegant font-semibold text-marrom-claro">{eventData.title.split(' - ')[0]}</h3>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="text-dourado hover:text-marrom-claro transition-colors"
            onClick={() => setShowCalendarModal(true)}
          >
            <CalendarPlus className="w-5 h-5" />
          </motion.button>
        </div>
        <div className="space-y-3 text-marrom-claro">
          <div className="flex items-center space-x-3">
            <Calendar className="w-5 h-5 text-dourado" />
            <span className="font-sans-modern">{formatEventDate(eventData.date)}</span>
          </div>
          <div className="flex items-center space-x-3">
            <Clock className="w-5 h-5 text-dourado" />
            <span className="font-sans-modern">{eventData.startTime} - {eventData.endTime} BRT</span>
          </div>
          <div className="flex items-center space-x-3">
            <MapPin className="w-5 h-5 text-dourado" />
            <span className="font-sans-modern">{eventData.location}</span>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="space-y-6 ">
          <div className="flex justify-between  items-center">
            <h3 className="text-xl font-serif-elegant font-semibold text-marrom-claro">Adicionar ao Calendário</h3>
            <motion.button
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
              onClick={() => setShowCalendarModal(false)}
              className="text-marrom-claro hover:text-dourado"
            >
              <X className="w-5 h-5" />
            </motion.button>
          </div>
          ...
          <div className="space-y-3">
            <CalendarButton
              icon={(props) => <Chrome {...props} className="w-5 h-5 text-dourado" />}
              label="Google Calendar"
              onClick={() => window.open(googleCalendarLink(), '_blank')}
            />

            <CalendarButton
              icon={(props) => <Apple {...props} className="w-5 h-5 text-marrom-claro" />}
              label="Apple Calendar"
              onClick={downloadICSFile}
            />

            <CalendarButton
              icon={(props) => <CalendarIcon {...props} className="w-5 h-5 text-verde-folha" />}
